...
| Método | Rota | Descrição |
|---|---|---|
| POST | `/inventario/bipagem` | Registra scan (UPDATE atômico + INSERT) |
...
| GET | `/inventario/historico/{cddeposito}` | Histórico de consolidações |
...
| Tabela | Uso |
|---|---|
| `INVENTARIO_TEMP` | Sessão de contagem em andamento |
| `OPERADORES_APP` | Operadores de coleta físicos |
| `LOG_INVENTARIO` | Auditoria completa de todas as operações |

---

## Segurança

- **JWT HS256** com expiração de 8h. `JWT_SECRET` no `.env`; se ausente, usa chave aleatória (aviso no console)
- **Rate limit** de login: 5 tentativas em 60s → bloqueio de 300s, por IP e por usuário
- **Licença RSA 2048-bit**: JWT RS256, chave pública embutida em `app/licenca.py`. Servidor não sobe sem licença válida
- **Supervisor obrigatório** quando há divergências: operadores precisam de gerente/admin diferente; gerentes podem autorizar a si mesmos
- **LOG_INVENTARIO** registra: `EDICAO`, `EDICAO_SUSPEITA`, `EXCLUSAO`, `ALERTA`, `ALERTA_REESCAN`, `CONSOLIDACAO`
